...
class Amazon
  ...
  def check_inventory
    ...
    item_ids = urls.select { |u| u.host == 'www.amazon.com' }.map { |u| u.path.match(/\/dp\/([\w]+)/)[1] }.compact.uniq.each_slice(10).to_a
  ...
  def get_items(item_ids:)
    ...
    if response.status == 200
      ...
      puts "[ERROR] #{response.status} – #{response.to_h.dig('Errors')&.map { |e| e.dig('Message') }&.join(', ')} – #{response.headers.to_h}"
    end
  end
